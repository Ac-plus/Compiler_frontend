...
int lineNum = 0;
bool lastWord = false;  // "-"前面一个字符是否是var、num或")"
bool neg = false;       // "-"是否充当负号使用
    ...
    */
    while((c = file.get()) != EOF) {
        if (c == ' ' || c == '\t' || c == '\n') {
            ...
            else if(operationBeginChar.count(c)) { // 有可能是两个字符组成的运算符
